...
export class ServicivimisterioService {

  private url = 'https://universoufologiamisterios.firebaseio.com/';

  constructor( private http: HttpClient, private db: AngularFirestore ) { }

  private civimisterio_img = 'civimisterio';

  crear ( crear: JdModel ){                     
    return this.http.post(`${ this.url }/civimisterio.json`, crear).pipe(    
  ...
  actualizar( actualizar: JdModel ){                                             
    ...
    const idTemporal = {                                                                   
      ...
      return this.http.put(`${ this.url }/civimisterio/${ actualizar.id }.json`, idTemporal)         
  }

  getUnElemento(id:string){                                                       
    return this.http.get(`${ this.url }/civimisterio/${ id }.json`); 
  }

  getTodosElementos(){                                                    

    return this.http.get(`${ this.url }/civimisterio.json`).pipe( map( this.variableCrearArreglo 
    ));
  }
  ...
  borrarElemento( VariableId: string ){                                             
    ...
    return this.http.delete(`${ this.url }/civimisterio/${ VariableId }.json`);   
  }

  private guardarImagen( imagen: { nombre: string, url: string } ) {   
    this.db.collection(`/${ this.civimisterio_img }`).add( imagen );     
}                                        
...
cargarImagenesFirebase ( imagenes:FileItem[] ) {                        
  ...
  const storageRef = firebase.storage().ref();                     
   ...
   for ( const item of imagenes ){                             
        ...
        const UploadTask: firebase.storage.UploadTask = storageRef.child(`${ this.civimisterio_img }/${ item.nombreArchivo }`).put( item.archivo );      
