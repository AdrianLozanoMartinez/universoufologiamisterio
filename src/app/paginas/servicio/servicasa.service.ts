...
export class ServicasaService {

  private url = 'https://universoufologiamisterios.firebaseio.com/';

  private casa_img = 'casa';

  constructor( private http: HttpClient, private db: AngularFirestore ) { }

  crear ( crear: JdModel ){                     
    return this.http.post(`${ this.url }/casa.json`, crear).pipe(    
  ...
  actualizar( actualizar: JdModel ){                                             
    ...
    const idTemporal = {                                                                   
      ...
      return this.http.put(`${ this.url }/casa/${ actualizar.id }.json`, idTemporal)         
  }

  getUnElemento(id:string){                                                       
    return this.http.get(`${ this.url }/casa/${ id }.json`); 
  }

  getTodosElementos(){                                                    

    return this.http.get(`${ this.url }/casa.json`).pipe( map( this.variableCrearArreglo 
    ));
  }
  ...
  borrarElemento( VariableId: string ){                                             
    ...
    return this.http.delete(`${ this.url }/casa/${ VariableId }.json`);   
  }

  private guardarImagen( imagen: { nombre: string, url: string } ) {   
    this.db.collection(`/${ this.casa_img }`).add( imagen );     
}                                        
...
cargarImagenesFirebase ( imagenes:FileItem[] ) {                        
  ...
  const storageRef = firebase.storage().ref();                     
   ...
   for ( const item of imagenes ){                             
        ...
        const UploadTask: firebase.storage.UploadTask = storageRef.child(`${ this.casa_img }/${ item.nombreArchivo }`).put( item.archivo );      
