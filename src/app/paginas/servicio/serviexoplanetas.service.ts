...
export class ServiexoplanetasService {

  private url = 'https://universoufologiamisterios.firebaseio.com/';

  private exoplanetas_img = 'exoplanetas';

  constructor( private http: HttpClient, private db: AngularFirestore ) { }

  crear ( crear: JdModel ){                     
    return this.http.post(`${ this.url }/exoplanetas.json`, crear).pipe(    
  ...
  actualizar( actualizar: JdModel ){                                             
    ...
    const idTemporal = {                                                                   
      ...
      return this.http.put(`${ this.url }/exoplanetas/${ actualizar.id }.json`, idTemporal)         
  }

  getUnElemento(id:string){                                                       
    return this.http.get(`${ this.url }/exoplanetas/${ id }.json`); 
  }

  getTodosElementos(){                                                    

    return this.http.get(`${ this.url }/exoplanetas.json`).pipe( map( this.variableCrearArreglo 
    ));
  }
  ...
  borrarElemento( VariableId: string ){                                             
    ...
    return this.http.delete(`${ this.url }/exoplanetas/${ VariableId }.json`);   
  }

  private guardarImagen( imagen: { nombre: string, url: string } ) {   
    this.db.collection(`/${ this.exoplanetas_img }`).add( imagen );     
}                                        
...
cargarImagenesFirebase ( imagenes:FileItem[] ) {                        
  ...
  const storageRef = firebase.storage().ref();                     
   ...
   for ( const item of imagenes ){                             
        ...
        const UploadTask: firebase.storage.UploadTask = storageRef.child(`${ this.exoplanetas_img }/${ item.nombreArchivo }`).put( item.archivo );      
