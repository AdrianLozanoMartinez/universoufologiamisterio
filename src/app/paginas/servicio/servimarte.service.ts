...
export class ServimarteService {

  private url = 'https://universoufologiamisterios.firebaseio.com/';

  private marte_img = 'marte';

  constructor( private http: HttpClient, private db: AngularFirestore ) { }

  crear ( crear: JdModel ){                     
    return this.http.post(`${ this.url }/marte.json`, crear).pipe(    
  ...
  actualizar( actualizar: JdModel ){                                             
    ...
    const idTemporal = {                                                                   
      ...
      return this.http.put(`${ this.url }/marte/${ actualizar.id }.json`, idTemporal)         
  }

  getUnElemento(id:string){                                                       
    return this.http.get(`${ this.url }/marte/${ id }.json`); 
  }

  getTodosElementos(){                                                    

    return this.http.get(`${ this.url }/marte.json`).pipe( map( this.variableCrearArreglo 
    ));
  }
  ...
  borrarElemento( VariableId: string ){                                             
    ...
    return this.http.delete(`${ this.url }/marte/${ VariableId }.json`);   
  }

  private guardarImagen( imagen: { nombre: string, url: string } ) {   
    this.db.collection(`/${ this.marte_img }`).add( imagen );     
}                                        
...
cargarImagenesFirebase ( imagenes:FileItem[] ) {                        
  ...
  const storageRef = firebase.storage().ref();                     
   ...
   for ( const item of imagenes ){                             
        ...
        const UploadTask: firebase.storage.UploadTask = storageRef.child(`${ this.marte_img }/${ item.nombreArchivo }`).put( item.archivo );      
