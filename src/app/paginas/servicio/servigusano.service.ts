...
export class ServigusanoService {

  private url = 'https://universoufologiamisterios.firebaseio.com/';

  private gusano_img = 'gusano';

  constructor( private http: HttpClient, private db: AngularFirestore ) { }

  crear ( crear: JdModel ){                     
    return this.http.post(`${ this.url }/gusano.json`, crear).pipe(    
  ...
  actualizar( actualizar: JdModel ){                                             
    ...
    const idTemporal = {                                                                   
      ...
      return this.http.put(`${ this.url }/gusano/${ actualizar.id }.json`, idTemporal)         
  }

  getUnElemento(id:string){                                                       
    return this.http.get(`${ this.url }/gusano/${ id }.json`); 
  }

  getTodosElementos(){                                                    

    return this.http.get(`${ this.url }/gusano.json`).pipe( map( this.variableCrearArreglo 
    ));
  }
  ...
  borrarElemento( VariableId: string ){                                             
    ...
    return this.http.delete(`${ this.url }/gusano/${ VariableId }.json`);   
  }

  private guardarImagen( imagen: { nombre: string, url: string } ) {   
    this.db.collection(`/${ this.gusano_img }`).add( imagen );     
}                                        
...
cargarImagenesFirebase ( imagenes:FileItem[] ) {                        
  ...
  const storageRef = firebase.storage().ref();                     
   ...
   for ( const item of imagenes ){                             
        ...
        const UploadTask: firebase.storage.UploadTask = storageRef.child(`${ this.gusano_img }/${ item.nombreArchivo }`).put( item.archivo );      
